...
static void EF_flatten_into(ExpandedObjectHeader *eohptr,
                void *result, Size allocated_size);

static const ExpandedObjectMethods EF_methods =
{
    EF_get_flat_size,
    EF_flatten_into
};

Size EF_get_flat_size(ExpandedObjectHeader *eohptr)
{
...
void EF_flatten_into(ExpandedObjectHeader *eohptr,
                void *result, Size allocated_size)
{
    ExpandedFingerprintHeader *efh = (ExpandedFingerprintHeader *) eohptr;
    ...
    encoded_size = encode_fingerprint(efh->data, (uint8_t *) VARDATA(result), allocated_size - VARHDRSZ);
...
make_expanded_fingerprint(FingerprintData *data, MemoryContext parentcontext)
{
    ...
    efh = (ExpandedFingerprintHeader *)
        MemoryContextAlloc(objcxt, sizeof(ExpandedFingerprintHeader));
